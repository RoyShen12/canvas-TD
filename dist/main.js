...
class Position {
    ...
    move(speedVec) {
        if (speedVec instanceof PolarVector) {
            const baseUint = new Vector(1, 0)
                .rotate(speedVec.theta)
                .multiply(speedVec.r);
            this.x += baseUint.x;
            this.y += baseUint.y;
        }
    ...
    outOfBoundary(boundaryTL, boundaryBR, epsilon = 0) {
        return (boundaryTL.x - this.x > epsilon ||
            boundaryTL.y - this.y > epsilon ||
            this.x - boundaryBR.x > epsilon ||
            this.y - boundaryBR.y > epsilon);
    }
    toString() {
...
class Vector extends Position {
    ...
    rotate(angle, center = { x: 0, y: 0 }) {
        return new Vector((this.x - center.x) * Math.cos(angle) -
            (this.y - center.y) * Math.sin(angle) +
            center.x, (this.x - center.x) * Math.sin(angle) +
...
class GemBase {
    ...
    get maxLevelHuman() {
        return isFinite(this.constructor.maxLevel)
            ? this.constructor.maxLevel + "  级"
            : "∞";
    }
    get levelUpPoint() {
        return 0;
...
class PainEnhancer extends GemBase {
    ...
    get bleedDamageRatio() {
        return (PainEnhancer.baseBleedDamageRatio +
            this.level * PainEnhancer.bleedDamageRatioLevelMx);
    }
...
class SimplicitysStrength extends GemBase {
    ...
    get attackAddition() {
        return (SimplicitysStrength.baseAttackAddition +
            this.level * SimplicitysStrength.attackAdditionLevelMx);
    }
...
class GogokOfSwiftness extends GemBase {
    ...
    get hasteAddition() {
        return (GogokOfSwiftness.baseHasteAddition +
            this.level * GogokOfSwiftness.hasteAdditionLevelMx);
    }
    initEffect(thisTower) {
        this.tower = thisTower;
...
class MirinaeTeardropOfTheStarweaver extends GemBase {
    ...
    get chitDamageRatio() {
        return (MirinaeTeardropOfTheStarweaver.baseChitDamageRatio +
            this.level * MirinaeTeardropOfTheStarweaver.chitDamageRatioLevelMx);
    }
    get description() {
    ...
    chit(thisTower, target) {
        target.health -=
            thisTower.Atk * this.chitDamageRatio * (1 - target.armorResistance);
...
class BaneOfTheStricken extends GemBase {
    ...
    get damageMakingRatioPerHit() {
        return (BaneOfTheStricken.baseDamageMakingRatio +
            this.level * BaneOfTheStricken.damageMakingRatioLevelMx);
    }
    get description() {
...
class GemOfMysterious extends GemBase {
    ...
    get pointAddition() {
        return (GemOfMysterious.basePointAddition +
            this.level * GemOfMysterious.pointAdditionLevelMx);
    }
    initEffect(thisTower) {
        this.tower = thisTower;
...
class BaneOfTheTrapped extends GemBase {
    ...
    get damageMakingRatioOnTrapped() {
        return (BaneOfTheTrapped.baseDamageMakingRatioOnTrapped +
            this.level * BaneOfTheTrapped.damageMakingRatioOnTrappedLevelMx);
    }
    initEffect(thisTower) {
        this.tower = thisTower;
...
class ZeisStoneOfVengeance extends GemBase {
    ...
    get damageMakingRatioMin() {
        return (ZeisStoneOfVengeance.baseDamageMakingRatioMin +
            this.level * ZeisStoneOfVengeance.damageMakingRatioLevelMxMin);
    }
    get damageMakingRatioMax() {
        return (ZeisStoneOfVengeance.baseDamageMakingRatioMax +
            this.level * ZeisStoneOfVengeance.damageMakingRatioLevelMxMax);
    }
    get description() {
        return ZeisStoneOfVengeance.__base_description
            .replace("$", (this.damageMakingRatioMin * 100).toFixed(2))
            .replace("$", (this.damageMakingRatioMax * 100).toFixed(2));
    }
    get levelUpPoint() {
...
ZeisStoneOfVengeance.baseDamageMakingRatioMax = 1.2;
...
class EchoOfLight extends GemBase {
    ...
    get extraTotalDamageRatio() {
        return (EchoOfLight.baseExtraTotalDamageRatio +
            this.level * EchoOfLight.extraTotalDamageRatioLevelMx);
    }
    get lightDotCount() {
    ...
    hitHook(thisTower, monster) {
        ...
        Tools.installDotDuplicated(monster, "beOnLightEcho", EchoOfLight.duration, this.Hst, Math.round((thisTower.Atk *
            critR *
            thisTower.calculateDamageRatio(monster) *
            this.extraTotalDamageRatio) /
            this.lightDotCount), false, thisTower.recordDamage.bind(thisTower));
    }
}
...
class GemOfAnger extends GemBase {
    ...
    get damageAdditionPerEnemy() {
        return (GemOfAnger.baseDamageAdditionPerEnemy +
            this.level * GemOfAnger.damageAdditionPerEnemyLevelMx);
    }
    tickHook(thisTower, monsters) {
        const inRangeCount = monsters.filter((mst) => thisTower.inRange(mst)).length;
        thisTower.__anger_gem_atk_ratio =
            this.damageAdditionPerEnemy * inRangeCount + 1;
    }
}
...
class BrokenPieces extends GemBase {
    ...
    get armorDecreaseStrength() {
        return (BrokenPieces.baseArmorDecreaseStrength +
            this.level * BrokenPieces.armorDecreaseStrengthLevelMx);
    }
    get armorBasedDamageRatio() {
        return (BrokenPieces.baseExtraArmorBasedDamageRatio +
            this.level * BrokenPieces.extraArmorBasedDamageRatioLevelMx);
    }
    get description() {
...
class Tools {
    static britishFormatter(num, precise = 1) {
        ...
        if (thisAbs < 1e3) {
            ...
            return (Tools.formatterUs.format(this.roundWithFixed(num / 1e12, precise)) +
                " T");
        }
    }
    ...
    static installDot(target, dotDebuffName, duration, interval, singleAttack, isIgnoreArmor, damageEmitter) {
        ...
        if (singleAttack === 0 || duration === 0) {
            ...
            const itv = setInterval(() => {
                ...
                if (target.health > 0) {
                    target.health -=
                        singleAttack * (isIgnoreArmor ? 1 : 1 - target.armorResistance);
...
class TowerBase extends ItemBase {
    constructor(position, radius, borderWidth, borderStyle, image, price, levelAtkFx, levelHstFx, levelSlcFx, levelRngFx) {
        ...
        Tools.ObjectFx.addFinalReadonlyProperty(this, "recordDamage", ({ lastAbsDmg, isDead, isBoss }) => {
            ...
            if (isDead) {
                ...
                Game.updateGemPoint +=
                    (isBoss
                        ? TowerBase.killBossPointEarnings
                        : TowerBase.killNormalPointEarnings) + this.__kill_extra_point;
                if (this.gem) {
    ...
    get Atk() {
        return (this.levelAtkFx(this.level) *
            this.__atk_ratio *
            this.__anger_gem_atk_ratio);
    }
    get Hst() {
        ...
        const base = [
            ...
            [
                ...
                this.isMaxLevel
                    ? "最高等级"
                    : "$ " +
                        Tools.formatterUs.format(Math.round(this.price[this.level + 1])),
            ],
    ...
    renderStatusBoard(bx1, _bx2, by1, _by2, showGemPanel, showMoreDetail, specifedWidth) {
        ...
        const renderDataType_1 = (rootNode, dataChunk, offset, showDesc) => {
            ...
            dataChunk.forEach((data, idx) => {
                const showD = showDesc &&
                    this.constructor.informationDesc.has(data[0]);
                ...
                if (data[0] === "售价" || data[0] === "类型") {
                    ...
                    if (this.isMaxLevel)
                        ...
                        row.lastChild.style.color =
                            this.price[this.level + 1] < Game.callMoney()[0] ? green : red;
                }
            });
        };
        ...
        const lineCount = this.informationSeq.length +
            this.descriptionChuned.length +
            this.exploitsSeq.length;
        const moreDescLineCount = showMoreDetail
            ? this.informationSeq.filter((f) => this.constructor.informationDesc.has(f[0])).length
            : 0;
        ...
        if (showGemPanel) {
            ...
            if (!this.gem) {
                ...
                select.onchange = () => {
                    ...
                    rowPrice.lastChild.textContent =
                        "$ " + Tools.formatterUs.format(ctor.price);
                    rowPrice.lastChild.style.color =
                        ctor.price <= Game.callMoney()[0] ? green : red;
                ...
                Tools.Dom.bindLongPressEventHelper(this.id + "", btn, () => {
                    if (!this.gem.isMaxLevel &&
                        Game.updateGemPoint >= this.gem.levelUpPoint) {
                        btn.onclick(null);
                        return false;
                    }
                    else {
                ...
                Tools.Dom.generateTwoCol(Tools.Dom.generateRow(gemElement), { textContent: "下一级点数" }, {
                    textContent: this.gem.isMaxLevel
                        ? "最高等级"
                        : Tools.formatterUs.format(this.gem.levelUpPoint),
        ...
        const positionTLX = this.position.x -
            (position === 1 || position === 4
                ? this.radius * -1
                : specifedWidth + this.radius);
        let positionTLY = this.position.y +
            (position === 1 || position === 2 ? -1 : 0) * (bHeight + this.radius);
...
TowerBase.informationDesc = new Map([
    [
        "等级",
        "鼠标单击图标或按 [C] 键来消耗金币升级，等级影响很多属性，到达某个等级可以晋升",
    ],
    ["下一级", "升级到下一级需要的金币数量"],
    ["售价", "出售此塔可以返还的金币数量"],
    ["伤害", "此塔的基础攻击力"],
...
class MonsterBase extends ItemBase {
    ...
    get informationSeq() {
        const base = [
            ...
            [
                "生命值",
                Tools.chineseFormatter(Math.round(this.__inner_current_health), 3) +
                    "/" +
                    Tools.chineseFormatter(Math.round(this.maxHealth), 3),
            ],
            ["移动速度", Tools.roundWithFixed(this.speedValue * 60, 1)],
            [
                "护甲",
                Tools.formatterUs.format(Math.round(this.inner_armor)) +
                    "（减伤 " +
                    Tools.roundWithFixed(this.armorResistance * 100, 1) +
                    "%）",
            ],
        ];
        return base;
    ...
    renderHealthBar(context) {
        if (this.health <= 0 || this.health / this.maxHealth > 1)
            return;
        const xaxisOffset = this.healthBarWidth < this.radius * 2
            ? 0
            : this.healthBarWidth / 2 - this.radius;
...
class BulletBase extends ItemBase {
    ...
    get isReaching() {
        return (Position.distancePow2(this.position, this.target.position) <
            Math.pow(this.target.radius + this.radius, 2));
    }
    inRange(target) {
...
class CannonBullet extends BulletBase {
    ...
    get isReaching() {
        if (this.aimPosition)
            return (Position.distancePow2(this.position, this.aimPosition) <
                Math.pow(20 + this.radius, 2));
    ...
    hit(monster, _magnification = 1, monsters) {
        ...
        monsters.forEach((m) => {
            if (Position.distancePow2(m.position, target) <
                this.explosionRadius * this.explosionRadius) {
                m.health -=
                    this.explosionDmg * (1 - m.armorResistance) * this.ratioCalc(m);
                this.emitter(m);
...
class NormalArrow extends BulletBase {
    ...
    hit(monster) {
        ...
        monster.health -=
            this.Atk * critMagnification * (1 - monster.armorResistance * 0.7);
        this.emitter(monster);
        if (this.willTrap) {
...
class PenetratingArrow extends BulletBase {
    constructor(position, atk, target, image) {
        ...
        this.destination = this.position
            .copy()
            .moveTo(this.target.position, Game.callDiagonalLength());
    }
    run(monsters) {
...
class CanvasManager {
    ...
    refreshText(text, context, positionTL, outerBoxPositionTL, width, height, style, fillOrStroke = true, font) {
        ...
        if (font)
            context.font = font;
        fillOrStroke
            ? context.fillText(text, positionTL.x, positionTL.y, width)
            : context.strokeText(text, positionTL.x, positionTL.y, width);
    }
}
class EventManager {
...
class MonsterManager {
    ...
    get maxLevel() {
        return this.monsters.length > 0
            ? _.maxBy(this.monsters, "__inner_level").level
            : 0;
    }
}
...
Swordman.spd = (lvl) => Math.min(0.3 + lvl / 60, 1.15);
...
Axeman.spd = (lvl) => Math.min(0.25 + lvl / 80, 1);
...
LionMan.spd = (lvl) => Math.min(0.38 + lvl / 70, 1.2);
...
class HighPriest extends MonsterBase {
    ...
    inHealingRange(target) {
        return (Position.distancePow2(target.position, this.position) <
            this.Hrng * this.Hrng);
    }
    healing(target) {
...
class _TowerManager {
    ...
    Factory(towerName, position, image, bulletImage, radius, ...extraArgs) {
        ...
        this.constructor.independentCtors.includes(nt.constructor.name)
    ...
    get totalDamage() {
        return this.towers
            .concat(this.independentTowers)
            .reduce((cv, pv) => cv + pv.__total_damage, 0);
    }
    get totalKill() {
        return this.towers
            .concat(this.independentTowers)
            .reduce((cv, pv) => cv + pv.__kill_count, 0);
    }
}
_TowerManager.independentCtors = ["_Jet"];
...
class CannonShooter extends TowerBase {
        ...
        if (ret !== 0) {
            switch (this.level) {
                ...
                case 90:
                    this.rankUp();
                    this.name = this.name
                        .replace(TowerManager.rankPostfixL1, TowerManager.rankPostfixL2)
                        .replace("V", "I");
    ...
    get EpdRng() {
        return this.reviceRange((this.levelEpdRngFx(this.level) + this.extraExplosionRange) *
            this.extraExplosionRangeRatio);
    }
    get EpdAtk() {
        return ((this.levelEpdAtkFx(this.Atk) + this.extraExplosionDamage) *
            this.extraExplosionDamageRatio);
    }
    get BrnAtk() {
...
class FrostTower extends TowerBase {
    ...
    rapidRender(context) {
        if (this.canFreeze)
            return;
        context.fillStyle = "rgba(25,25,25,.3)";
        Tools.renderSector(context, this.position.x, this.position.y, this.radius, 0, Math.PI *
            2 *
            (1 - (performance.now() - this.lastFreezeTime) / this.freezeInterval), false).fill();
    }
}
...
class TeslaTower extends TowerBase {
    ...
    calculateRandomCirclePoint() {
        ...
        return {
            ...
            y: Math.random() > 0.5
                ? this.position.y -
                    Math.pow(this.Rng * this.Rng - Math.pow(x - this.position.x, 2), 0.5)
    ...
    rapidRender(ctx, monsters) {
        ...
        ctx.beginPath();
        this.monsterShockingRenderingQueue =
            this.monsterShockingRenderingQueue.filter((msrq) => {
...
class BlackMagicTower extends TowerBase {
    ...
    get informationSeq() {
        return super.informationSeq.concat([
            ...
            [
                "额外攻击速度",
                Tools.roundWithFixed(this.imprecationHaste * 100 - 100, 1) + "%",
            ],
        ]);
    }
    levelUp(currentMoney) {
    ...
    produceBullet() {
        ...
        this.target.health -=
            this.Atk * this.calculateDamageRatio(this.target) +
                this.target.health * this.POTCHD;
...
BlackMagicTower.deniedGems = ["GogokOfSwiftness"];
class _ColossusLaser {
    constructor(startPos, endPos, lineWidth, duration, swipeVector, easingFunc, ls1, ls2) {
        ...
        const maxAnimationCount = Math.floor(this.swipeVector.r /
            (Math.max(_ColossusLaser.animationW, _ColossusLaser.animationH) + 1));
        this.animationStepInterval =
            maxAnimationCount >= updateCount
                ? 1
                : Math.ceil(updateTime / maxAnimationCount);
...
_ColossusLaser.animationSpeed = 0.5;
class LaserTower extends TowerBase {
    ...
    produceBullet(_i, monsters) {
        ...
        this.target.health -=
            this.Atk *
                (1 - this.target.armorResistance) *
                this.calculateDamageRatio(this.target);
        this.recordDamage(this.target);
        monsters.forEach((mst) => {
            if (Game.callCanvasContext("bg").isPointInPath(flameArea, mst.position.x, mst.position.y)) {
                ...
                mst.health -=
                    this.Fatk *
                        (1 - mst.armorResistance) *
                        this.calculateDamageRatio(mst);
                this.recordDamage(this.target);
            }
        });
    }
...
class _Jet extends TowerBase {
    ...
    get attackSupplement() {
        return this.weaponMode === 1
            ? this.carrierTower.Atk * -0.55
            : Math.pow(this.level + 2, 2.225) * 6;
    }
    ...
    run(monsters) {
        switch (this.actMode) {
            ...
            case _Jet.JetActMode.f1:
                super.run(monsters);
                if (Position.distance(this.position, this.destinationPosition) >
                    this.radius * 2) {
...
_Jet.JetWeapons = {
    getCtorName(mode) {
        return mode === 1
            ? "CarrierTower.Jet.JetWeapons.MachineGun"
            : "CarrierTower.Jet.JetWeapons.AutoCannons";
    },
    MachineGun: class _MachineGun extends BulletBase {
        ...
        hit(monster, magnification = 1) {
            monster.health -=
                this.Atk * magnification * (1 - monster.armorResistance * 0.65);
            this.emitter(monster);
        }
    },
...
CarrierTower.deniedGems = ["ZeisStoneOfVengeance", "GemOfAnger"];
...
class EjectBlade extends TowerBase {
    ...
    get informationSeq() {
        return super.informationSeq.concat([
            ...
            [
                "弹射伤害系数",
                Tools.roundWithFixed(this.damageFadePerBounce * 100, 2) + " %",
            ],
        ]);
    }
    get bounceTime() {
...
class AnimationSprite extends Base {
    ...
    render(context, positionTL, width, height, delay = 0, callback) {
        if (this.realNextFrameIndex !== 0 ||
            this.realNextFrameIndex !== this.totalFrame) {
...
class Game extends Base {
    constructor(imageManager, GX = 36, GY = 24) {
                ...
                this.grids.forEach((rowv, rowi) => {
                    rowv.forEach((colv, coli) => {
                        if (!(mouseGposx === rowi && mouseGposy === coli) &&
                            colv === 1 &&
                            !(rowi === this.DestinationGrid.x - 1 &&
                                coli === this.DestinationGrid.y - 1)) {
        ...
        this.mouseMoveHandler = _.throttle((e) => {
            ...
            if (e.offsetX > this.midSplitLineX) {
                ...
                const selectedT = this.towerCtl.towers.find((t) => t.position.equal(mousePos, t.radius)) ||
                    this.towerCtl.independentTowers.find((t) => t.position.equal(mousePos, t.radius));
        ...
        Game.callChangeF1Mode = (v) => {
            this.towerCtl.independentTowers.forEach((t) => {
                t.actMode = v
                    ? CarrierTower.Jet.JetActMode.f1
                    : CarrierTower.Jet.JetActMode.autonomous;
            });
        };
        Game.callChangeCarrierWeaponMode = (v) => {
            this.towerCtl.independentTowers.forEach((t) => {
    ...
    coordinateToGridIndex(pos) {
        ...
        return [
            Math.max(Math.floor(rubbed[1] / this.gridSize), 0),
            Math.max(Math.floor(rubbed[0] / this.gridSize), 0),
        ];
    }
    whichGrid(pos) {
    ...
    getPathToEnd(startPos) {
        ...
        if (this.posPathMapping.has(key)) {
            ...
            const path = Astar.astar
                .search(G, G.grid[wg[0] + 1][wg[1] + 1], G.grid[this.DestinationGrid.x][this.DestinationGrid.y])
                .map((p, _idx) => {
    ...
    placeMonster(level, pos, ctorName) {
        ...
        this.monsterCtl.Factory(ctorName, pos.dithering(this.gridSize / 3), imgName.indexOf("$spr::") !== -1
            ? this.imageCtl.getSprite(imgName.substr(6)).getClone(sprSpd || 1)
            : this.imageCtl.getImage(imgName), level);
    }
    ...
    initButtons() {
        ...
        this.startAndPauseButton = new ButtonOnDom({
            ...
            onclick: () => {
                ...
                this.startAndPauseButton.ele.textContent = this.isPausing
                    ? "开始"
                    : "暂停";
            },
        });
    ...
    renderOnce() {
        ...
        chunkedTowerCtors.forEach((ctorRow, rowIdx) => {
            rowIdx > 0
                ? tsAeraRectTL.move(new PolarVector(tsMarginBottom + tsItemRadius * 2, 270))
                : void 0;
    ...
    update() {
        this.updateTick++;
        if (!this.isPausing &&
            !Reflect.get(window, "__d_stop_ms") &&
            !this.__dummy_test_mode) {
    ...
    render(towerNeedRender = true) {
        ...
        if (this.__testMode) {
            ...
            if (actualLength < 60) {
                this.averageFrameInterval =
                    (now - this.renderTimeStamps[0]) / actualLength;
            }
            else {
                this.averageFrameInterval =
                    (now - this.renderTimeStamps[actualLength - 60]) / 60;
            }
    ...
    renderInformation() {
        const DPS = this.bornStamp
            ? Tools.chineseFormatter((this.towerCtl.totalDamage / (performance.now() - this.bornStamp)) *
                1000, 3, " ")
            : 0;
