...
class MirinaeTeardropOfTheStarweaver extends GemBase {
    ...
    chance = MirinaeTeardropOfTheStarweaver.chance;
    get chitDamageRatio() {
    ...
    Hst = MirinaeTeardropOfTheStarweaver.Hst;
    get description() {
...
class BaneOfTheStricken extends GemBase {
    ...
    damageMakingRatioOnBoss = BaneOfTheStricken.damageMakingRatioOnBoss;
    get description() {
...
class ZeisStoneOfVengeance extends GemBase {
    ...
    chance = ZeisStoneOfVengeance.chance;
...
class EchoOfLight extends GemBase {
    ...
    hitHook(thisTower, monster) {
        let critR = 1;
        if (thisTower.critChance != null && thisTower.critDamageRatio != null && Math.random() < thisTower.critChance) {
            critR = thisTower.critDamageRatio;
        }
        Tools.installDotDuplicatable(monster, 'beOnLightEcho', EchoOfLight.duration, this.Hst, Math.round((thisTower.Atk * critR * thisTower.calculateDamageRatio(monster) * this.extraTotalDamageRatio) / this.lightDotCount), false, thisTower.recordDamage.bind(thisTower));
    }
}
...
class TowerBase extends ItemBase {
    ...
    target = null;
    lastShootTime;
    __inner_total_damage = 0;
    __inner_kill_count = 0;
    gem = null;
    canInsertGem = true;
    __hst_ps_ratio = 1;
    __atk_ratio = 1;
    __kill_extra_gold = 0;
    __kill_extra_point = 0;
    __on_boss_atk_ratio = 1;
    __on_trapped_atk_ratio = 1;
    __anger_gem_atk_ratio = 1;
    __max_rng_atk_ratio = 1;
    __min_rng_atk_ratio = 1;
    __each_monster_damage_ratio = new Map();
    description = null;
    name = null;
    isSold = false;
    __grid_ix;
    ...
    constructor(position, radius, borderWidth, borderStyle, image, price, levelAtkFx, levelHstFx, levelSlcFx, levelRngFx) {
        ...
        this.intervalTimers.push(setInterval(() => {
            const monsters = Game.callMonsterList();
    ...
    get informationSeq() {
        const base = [
            [this.name || '', ''],
            ['等级', this.levelHuman],
            ['下一级', this.isMaxLevel ? '最高等级' : '$ ' + Tools.formatterUs.format(Math.round(this.price[this.level + 1] || 0))],
    ...
    renderLevel(context) {
        ...
        context.fillText('lv ' + this.levelHuman, this.position.x + this.radius * 0.78, this.position.y - this.radius * 0.78);
        context.font = f_tmp;
    }
    ...
    renderStatusBoard(bx1, _bx2, by1, _by2, showGemPanel, showMoreDetail, specifiedWidth) {
        ...
        const renderDataType_1 = (rootNode, dataChunk, offset, showDesc) => {
            ...
            dataChunk.forEach((data, idx) => {
                ...
                if (!row.hasChildNodes()) {
                    Tools.Dom.generateTwoCol(row, null, null);
                }
                else {
        ...
        const moreDescLineCount = showMoreDetail ? this.informationSeq.filter(info => this.constructor.informationDesc.has(info[0])).length : 0;
        ...
        if (showGemPanel) {
            ...
            if (!this.gem) {
                ...
                const rowImage = Tools.Dom.generateRow(gemElement);
                const ctor = TowerBase.GemNameToGemCtor(selected);
                ...
                Tools.Dom.generateRow(gemElement, null, undefined, [btn]);
            }
            else {
                ...
                Tools.Dom.bindLongPressEventHelper(this.id + '', btn, () => {
                    if (this.gem && !this.gem.isMaxLevel && Game.updateGemPoint >= this.gem.levelUpPoint) {
                ...
                Tools.Dom.generateTwoCol(Tools.Dom.generateRow(gemElement), { textContent: '下一级点数' }, {
                    textContent: this.gem.isMaxLevel ? '最高等级' : Tools.formatterUs.format(this.gem.levelUpPoint),
                    style: { color: canUpdateNext ? green : red }
                });
                Tools.Dom.generateRow(gemElement, null, {
        ...
        let position = 2;
        if (this.position.x - bx1 < specifiedWidth + this.radius) {
            position = 1;
        ...
        const positionTLX = this.position.x - (position === 1 || position === 4 ? this.radius * -1 : specifiedWidth + this.radius);
...
class MonsterBase extends ItemBase {
    ...
    constructor(position, radius, borderWidth, borderStyle, image, level, levelRwdFx, levelSpdFx, levelHthFx, levelAmrFx, levelShdFx) {
        ...
        this.exploitsSeq = [['赏金', Tools.chineseFormatter(this.reward, 0)]];
    }
    get beImprecated() {
...
class CannonBullet extends BulletBase {
    ...
    burnDotDuration;
    ratioCalc;
    constructor(position, atk, target, _b_img, explosionDmg, explosionRadius, burnDotDamage, burnDotInterval, burnDotDuration, extraBV, ratioCalc) {
...
class CanvasManager {
    ...
    getContext(id) {
        if (this.canvasContextMapping.has(id))
            ...
            throw new Error('cannot get canvasContext as id ' + id);
    }
    createCanvasInstance(id, style = {}, height, width, offDocument, wiredEvent = null, paintingOffScreenRenderingContextId = null) {
        ...
        if (offDocument) {
            ...
            canvasEle.id = id;
            if (wiredEvent && Object.prototype.toString.call(wiredEvent) === '[object Function]') {
...
const towerCtors = [
    ...
    },
    {
        ...
        h: (_lvl) => Infinity,
        s: (_lvl) => 0,
        sr: (lvl) => Tools.MathFx.naturalLogFx(0.1, 0.12)(lvl)
    },
    {
        dn: '毒气塔',
        c: 'PoisonTower',
...
class TestTower extends TowerBase {
    ...
    constructor(position, image, _b_image, radius) {
...
class CannonShooter extends TowerBase {
    ...
    constructor(position, image, _b_img, radius) {
...
class MaskManTower extends TowerBase {
    ...
    critDamageRatio = 2;
    secKillChance = 0;
    constructor(position, image, b_image, radius) {
...
class TeslaTower extends TowerBase {
    static renderLighteningCop(ctx, x1, y1, x2, y2, displace) {
        if (displace < TeslaTower.curveDetail) {
            ...
            const mid_x = (x2 + x1) / 2 + (Math.random() - 0.5) * displace;
            const mid_y = (y2 + y1) / 2 + (Math.random() - 0.5) * displace;
    ...
    extraHaste = 0;
    shockDuration = 5000;
    shockChargingChance = 0.2;
    shockChargingPowerRatio = 0.25;
    shockLeakingChance = 0.02;
        ...
        if (ret !== 0) {
            switch (this.level) {
                case 12:
                    ...
                    this.shockChargingChance = 0.3;
                    this.shockDuration = 8000;
                    this.shockChargingPowerRatio = 0.75;
                    this.shockLeakingChance = 0.05;
                    break;
                case 24:
                    this.rankUp();
                    this.name = '闪电风暴塔';
                    ...
                    this.extraRange = 80;
                    this.shockChargingChance = 0.4;
                    this.shockDuration = 12000;
                    this.shockChargingPowerRatio = 1.5;
                    this.shockLeakingChance = 0.12;
                    break;
            }
        }
        return ret;
...
class LaserTower extends TowerBase {
    ...
    produceBullet(_i, monsters) {
        if (this.target) {
            ...
            const swipeVector = _.maxBy(_.range(0, 360, 30).map(d => new PolarVector(this.LSD, d)), sv => monsters.filter(mst => v
                .copy()
                .move(sv.normalize().multiply(mvrc * (arcTime - 1) * r))
                .equal(mst.position, 1.2 * r)).length).dithering((1 / 30) * Math.PI);
            ...
            this.recordDamage(this.target);
            monsters.forEach(mst => {
                if (Game.callCanvasContext('bg').isPointInPath(flameArea, mst.position.x, mst.position.y) && this.target) {
...
class _Jet extends TowerBase {
    ...
    carrierTower;
    constructor(position, image, _b_img, radius, carrierTower) {
...
class EjectBlade extends TowerBase {
    ...
    constructor(position, image, b_img, radius) {
...
class DamageTextBox {
    ...
    width;
    fontStyle;
    constructor(text, pos, width = 120, fontSize, minFontSize, fillStyle, speed, lifeTick) {
        this.position = pos;
        this.width = width;
        this.damage = text;
...
class AnimationSprite extends Base {
    ...
    renderOneFrame(context, positionTL, width, height, delay, endless, trusteeShippedClearing, recirculation, callback) {
        if (this.isFinish) {
            if (endless) {
                this.nextFrameIndex = 0;
            }
    ...
    renderLoop(context, positionTL, width, height, trusteeShippedClearing = false) {
        if (this.realNextFrameIndex !== 0 || this.realNextFrameIndex !== this.totalFrame) {
            if (this.lastRAF)
                cancelAnimationFrame(this.lastRAF);
        }
...
class HostedAnimationSprite extends Base {
    ...
    waitFrame;
    render;
    constructor(sp, pos, w, h, delay = 0, endless, waitFrame = 0) {
        super();
        this.sp = sp;
        this.waitFrame = waitFrame;
        this.render = function (ctx) {
            if (this.waitFrame === 0)
                this.sp.renderOneFrame(ctx, pos, w, h, delay, endless, true, false);
            else
                this.waitFrame--;
        };
    }
...
class Game extends Base {
    ...
    OriginGrid;
    DestinationGrid;
    __inner_is_pausing = true;
    __inner_b_arr;
    __update_speed_ratio = 1;
    money;
    life;
    towerForSelect = [];
    selectedTowerTypeToBuild = null;
    ...
    ctxBg;
    updateGemPoint;
    startAndPauseButton = null;
    speedControlButton = null;
    loopSpeeds;
    lastMouseMovePosition = Position.O;
    grids = [];
    posPathMapping = new Map();
    midSplitLineX = -1;
    detailFunctionKeyDown = false;
    useClassicRenderStyle;
    gridSize;
    leftAreaWidth;
    OriginPosition;
    DestinationPosition;
    ...
    get updateSpeedRatio() {
        return this.__update_speed_ratio;
    }
    set updateSpeedRatio(v) {
        ...
        switch (e.key) {
            ...
            case 'v':
                if (this.__testMode) {
                    ...
                    this.placeMonster(100, this.OriginPosition.copy()
                        .move(new PolarVector(this.gridSize, 0))
                        .dithering(this.gridSize, this.gridSize / 2), 'Dummy');
                }
                break;
            case 'b':
                if (this.__testMode) {
        ...
        if (this.__testMode) {
            ...
            Tools.Dom.generateRow(document.body, null, {
                ...
                Tools.Dom.__installOptionOnNode(document.createElement('input'), {
                    ...
                    value: 'on',
                    onchange: (e) => {
                        __debug_show_refresh_rect = e.target.checked;
                    }
                })
            ]);
        }
    ...
    renderOnce() {
        ...
        this.imageCtl
            .getSprite('gold_spin')
            .getClone(2)
            .renderLoop(this.ctxBg, new Position(innerWidth - 190, innerHeight - 25), 18, 18);
        ...
        this.imageCtl
            .getSprite('sparkle')
            .getClone(10)
            .renderLoop(this.ctxBg, new Position(innerWidth - 190, innerHeight - 85), 18, 18);
    ...
    performanceWrapper(fn, metricsName, xAxis, yAxis, bufferLength = 120, height = 50, maxWidth = 120, maxV = 50, warningV = 12, dangerV = 16.667) {
        ...
        return () => {
            ...
            Tools.renderStatistic(metricsName, this.ctxBg, timeBuffer, new Position(xAxis, yAxis + height), timeBuffer.length, height + 2, maxV, warningV, dangerV);
        };
    }
    run() {
    ...
    render(towerNeedRender = true) {
        ...
        if (this.ctxMain._off_screen_paint)
            this.ctxMain._off_screen_paint();
        else
            throw new Error('this.ctxMain._off_screen_paint is not a function');
        if (this.__testMode) {
