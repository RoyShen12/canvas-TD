...
const __global_test_mode = DEBUG_CONFIG.testMode;
let __debug_show_refresh_rect = DEBUG_CONFIG.showRefreshRect;
let __debug_black_magic_tower_always_enhance = DEBUG_CONFIG.blackMagicTowerAlwaysEnhance;
...
class DirtyRectManager {
    ...
    markDirtyByPosition(position, radius, padding = 5) {
        const size = radius * 2 + padding * 2;
        this.markDirty(position.x - radius - padding, position.y - radius - padding, size, size);
    }
    markFullRedraw() {
    ...
    getDirtyRects(canvasWidth, canvasHeight) {
    ...
    isPointDirty(x, y) {
        if (this.fullRedrawFlag)
            return true;
        return this.rects.some(rect => x >= rect.x && x <= rect.x + rect.width &&
...
class GameErrorHandler {
    ...
    async wrapAsync(fn, context, fallback) {
        try {
            return await fn();
        }
        catch (error) {
...
const TowerRegistry = new ClassRegistry();
const MonsterRegistry = new ClassRegistry();
const BulletRegistry = new ClassRegistry();
class MathUtils {
    ...
    static compareProperties(properties, cFx, ascend = true) {
        const compareFn = cFx ?? ((a, b) => a - b);
        return (a, b) => compareFn(a[properties], b[properties]) * (ascend ? 1 : -1);
    }
        ...
        for (const key of keys) {
            ...
            if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean' || typeof value === 'function') {
                ...
                for (const subKey of subKeys) {
                    node[key][subKey] = value[subKey];
                }
            }
        }
        return node;
    }
    static generateDiv(node, option) {
        option = option || {};
...
class RenderUtils {
    static renderStatisticInitialized = new Set();
    ...
    static renderStatistic(name, ctx, dataArr, positionTL, width, height, maxV, warningV, dangerV, transparency, goodColor, warningColor, dangerColor) {
        ...
        dataArr.forEach((v, i) => {
            ...
            if (v > dangerV) {
                if (ctx.fillStyle !== dangerColor && dangerColor)
                    ctx.fillStyle = dangerColor;
            }
            else if (v > warningV) {
                if (ctx.fillStyle !== warningColor && warningColor)
...
class DOTManager {
    ...
    static installDOT(target, dotDebuffName, duration, interval, singleAttack, isIgnoreArmor, damageEmitter) {
        ...
        target.intervalTimers.push(itv);
    }
    static installDotDuplicatable(target, dotDebuffName, duration, interval, singleAttack, isIgnoreArmor, damageEmitter) {
        const targetRecord = target;
...
class PolarVector {
    ...
    dithering(thetaAmp, rAmp) {
        ...
        if (rAmp)
            this.r += MathUtils.randomSig() * Math.random() * rAmp;
        return this;
    }
    multiply(f) {
        this.r *= f;
...
class StatusBoardRenderer {
    static renderDataType1(rootNode, dataChunk, offset, showDesc, constructorRef, isMaxLevel, price, level, getMoney) {
        let jump = 0;
        dataChunk.forEach((data, idx) => {
            ...
            if (!row.hasChildNodes()) {
                ...
                DomUtils.removeNodeTextAndStyle(row.lastChild);
                DomUtils.removeNodeTextAndStyle(row.firstChild);
            }
            ...
            if (data[0] === '售价' || data[0] === '类型') {
                ;
                row.lastChild.style.color = STATUS_BOARD_COLORS.DARK_GRAY;
                ...
                if (isMaxLevel) {
                    ;
                    row.lastChild.style.color = STATUS_BOARD_COLORS.LIGHT_GRAY;
                }
                else {
                    ;
                    row.lastChild.style.color =
    ...
    static render(data, bx1, by1, showGemPanel, showMoreDetail, specifiedWidth = 150, getElement, getMoney, updateGemPoint, setUpdateGemPoint) {
        showGemPanel = showGemPanel && data.canInsertGem;
        const blockElement = getElement('status_block');
        ...
        if (blockElement.childNodes.length > lineCount + extraLineCount) {
            blockElement.childNodes.forEach((child, index) => {
                if (index > lineCount - 1 + extraLineCount) {
                    ...
                    DomUtils.removeNodeTextAndStyle(child);
                }
            });
        }
        ...
        this.renderDataType1(blockElement, data.informationSeq, 0, showMoreDetail, data.constructorRef, data.isMaxLevel, data.price, data.level, getMoney);
        this.renderDivision(blockElement, l1 - 1);
        this.renderDataType1(blockElement, data.exploitsSeq, l1, false, data.constructorRef, data.isMaxLevel, data.price, data.level, getMoney);
        this.renderDivision(blockElement, l2 - 1);
        this.renderDataType2(blockElement, data.descriptionChunked, l2);
        if (showGemPanel) {
            this.renderGemPanel(data, bx1, by1, showMoreDetail, specifiedWidth, getElement, getMoney, updateGemPoint, setUpdateGemPoint);
        }
        else {
            const gemElement = getElement('gem_block');
            gemElement.style.display = 'none';
        }
        this.calculateAndApplyPosition(data, bx1, by1, showGemPanel, specifiedWidth, getElement);
    }
    static renderGemPanel(data, bx1, by1, showMoreDetail, specifiedWidth, getElement, getMoney, updateGemPoint, setUpdateGemPoint) {
        const gemElement = getElement('gem_block');
    ...
    static renderGemSelection(data, bx1, by1, showMoreDetail, specifiedWidth, getElement, getMoney, _updateGemPoint, _setUpdateGemPoint) {
        const gemElement = getElement('gem_block');
        ...
        const rowImage = DomUtils.generateRow(gemElement);
        const ctor = TowerBase.GemNameToGemCtor(selected);
        DomUtils.generateImg(rowImage, ctor.imgSrc, { className: 'lg_gem_img' });
        const rowPrice = DomUtils.generateRow(gemElement, null, { style: { marginBottom: '5px' } }, ctor.priceSpan);
        ...
        select.onchange = () => {
            ...
            const newCtor = TowerBase.GemNameToGemCtor(selected);
            rowDesc.textContent = newCtor.stasisDescription;
            rowImage.firstChild.src = newCtor.imgSrc;
    ...
    static renderGemDisplay(data, bx1, by1, showMoreDetail, specifiedWidth, getElement, getMoney, updateGemPoint, setUpdateGemPoint) {
        const gemElement = getElement('gem_block');
        const gem = data.gem;
        ...
        const [imgCol] = DomUtils.generateTwoCol(DomUtils.generateRow(gemElement, null, { style: { marginBottom: '5px' } }), null, null, [], [btn]);
        DomUtils.generateImg(imgCol, gem.imgSrc, { className: 'lg_gem_img' });
        DomUtils.generateRow(gemElement, null, {
            textContent: gem.level + '  级 / ' + gem.maxLevelHuman
        });
        DomUtils.generateTwoCol(DomUtils.generateRow(gemElement), { textContent: '下一级点数' }, {
    ...
    static calculateAndApplyPosition(data, bx1, by1, showGemPanel, specifiedWidth, getElement) {
        const blockElement = getElement('status_block');
        const gemElement = getElement('gem_block');
...
class GemBase {
    ...
    static maxLevel = 200;
    static stasisDescription = '';
    level = 0;
    tower = null;
    get ctor() {
        return this.constructor;
    }
    formatDescription(template, ...values) {
        let result = template;
    ...
    initEffect(_thisTower) { }
    attackHook(_thisTower, _monsters) { }
...
class PainEnhancer extends GemBase {
    static gemName = '增痛宝石';
    static price = 25000;
    static maxLevel = 1000;
    static imgSrc = GEM_IMAGES.PainEnhancer;
    static chance = 0.5;
    static baseBleedDamageRatio = 1250;
    static bleedDamageRatioLevelMx = 750;
    static get stasisDescription() {
        return `攻击敌人有 ${this.chance * 100}% 的几率使其流血，在 3 秒内受到 ${MathUtils.roundWithFixed(this.baseBleedDamageRatio * 100, 1)}% （+${MathUtils.roundWithFixed(this.bleedDamageRatioLevelMx * 100, 1)}%/等级）攻击力的伤害`;
    }
    static get __base_description() {
        return `攻击敌人有 ${this.chance * 100}% 的几率使其流血，在 3 秒内受到 $% 攻击力的伤害`;
    }
    bleedDuration = GEM_CONSTANTS.DOT_DEFAULT_DURATION;
    bleedInterval = GEM_CONSTANTS.DOT_DEFAULT_INTERVAL;
    get description() {
...
class SimplicitysStrength extends GemBase {
    static gemName = '至简之力';
    static price = 18000;
    static maxLevel = 10000;
    static imgSrc = GEM_IMAGES.SimplicitysStrength;
    static baseAttackAddition = 0.55;
    static attackAdditionLevelMx = 0.2;
    static get stasisDescription() {
...
class GogokOfSwiftness extends GemBase {
    ...
    static maxLevel = 800;
    static price = 50000;
    static imgSrc = GEM_IMAGES.GogokOfSwiftness;
    static baseHasteAddition = 0.15;
    static hasteAdditionLevelMx = 0.05;
    static get stasisDescription() {
...
class MirinaeTeardropOfTheStarweaver extends GemBase {
    static gemName = '银河、织星者之泪';
    static price = 120000;
    static maxLevel = 1000;
    static imgSrc = GEM_IMAGES.MirinaeTeardropOfTheStarweaver;
    static chance = 0.1;
    static baseChitDamageRatio = 100;
    static chitDamageRatioLevelMx = 7.5;
    static Hst = 1000;
    static levelRequirement = 25;
    static get stasisDescription() {
        return `击中时有 ${this.chance * 100}% 的几率 [重击] 附近的一名敌人，每 ${this.Hst / 1000} 秒 [重击] 一名随机敌人 (需要${this.levelRequirement}级) [重击]: 对目标造成 ${MathUtils.roundWithFixed(this.baseChitDamageRatio * 100, 1)}%（+${MathUtils.roundWithFixed(this.chitDamageRatioLevelMx * 100, 1)}%/等级）攻击力的伤害`;
    }
    static get __base_description() {
...
class BaneOfTheStricken extends GemBase {
    static gemName = '受罚者之灾';
    static price = 65000;
    static imgSrc = GEM_IMAGES.BaneOfTheStricken;
    static maxLevel = 50000;
    static damageMakingRatioOnBoss = 0.75;
    static baseDamageMakingRatio = 0.0008;
    static damageMakingRatioLevelMx = 0.00005;
    static get stasisDescription() {
        return `你对敌人造成的每次攻击都会使敌人从你攻击中受到的伤害提高 ${MathUtils.roundWithFixed(this.baseDamageMakingRatio * 100, 2)}%（+${MathUtils.roundWithFixed(this.damageMakingRatioLevelMx * 100, 3)}%/等级），对首领造成的伤害提高 ${MathUtils.roundWithFixed(this.damageMakingRatioOnBoss * 100, 0)}%`;
    }
    static get __base_description() {
...
class GemOfEase extends GemBase {
    static gemName = '屯宝者的恩惠';
    static price = 10000;
    static maxLevel = 50;
    static imgSrc = GEM_IMAGES.GemOfEase;
    static baseGoldAddition = 500;
    static goldAdditionLevelMx = 150;
    static get stasisDescription() {
...
class GemOfMysterious extends GemBase {
    ...
    static price = 950000;
    static maxLevel = 50;
    static imgSrc = GEM_IMAGES.GemOfMysterious;
    static basePointAddition = 4000;
    static pointAdditionLevelMx = 800;
    static get stasisDescription() {
...
class BaneOfTheTrapped extends GemBase {
    static gemName = '囚笼宝石';
    static price = 20500;
    static imgSrc = GEM_IMAGES.BaneOfTheTrapped;
    static baseDamageMakingRatioOnTrapped = 5.85;
    static damageMakingRatioOnTrappedLevelMx = 1.25;
    static get stasisDescription() {
        return `对受到控制类限制效果影响的敌人造成的伤害提高 ${MathUtils.roundWithFixed(this.baseDamageMakingRatioOnTrapped * 100, 1)}%（+${MathUtils.roundWithFixed(this.damageMakingRatioOnTrappedLevelMx * 100, 1)}%/等级）`;
    }
    static get __base_description() {
...
class ZeisStoneOfVengeance extends GemBase {
    static gemName = '贼神的复仇之石';
    static price = 17500;
    static maxLevel = 500;
    static imgSrc = GEM_IMAGES.ZeisStoneOfVengeance;
    static baseDamageMakingRatioMin = 0.02;
    static damageMakingRatioLevelMxMin = 0.001;
    static baseDamageMakingRatioMax = 1.2;
    static damageMakingRatioLevelMxMax = 0.125;
    static chance = 0.2;
    static stuporDuration = 1000;
    static get stasisDescription() {
        return `你和命中的敌人间隔越远，造成的伤害就越高，最少提高 ${MathUtils.roundWithFixed(this.baseDamageMakingRatioMin * 100, 2)}%（+${MathUtils.roundWithFixed(this.damageMakingRatioLevelMxMin * 100, 2)}%/等级），在最远射程时提高 ${MathUtils.roundWithFixed(this.baseDamageMakingRatioMax * 100, 2)}%（+${MathUtils.roundWithFixed(this.damageMakingRatioLevelMxMax * 100, 2)}%/等级），击中时有 ${this.chance * 100}% 的几率使敌人昏迷 ${this.stuporDuration / 1000} 秒`;
    }
    static get __base_description() {
    ...
    get description() {
        return this.formatDescription(ZeisStoneOfVengeance.__base_description, (this.damageMakingRatioMin * 100).toFixed(2), (this.damageMakingRatioMax * 100).toFixed(2));
    }
    get levelUpPoint() {
...
class EchoOfLight extends GemBase {
    ...
    static price = 500000;
    static maxLevel = 1000;
    static imgSrc = GEM_IMAGES.EchoOfLight;
    static baseExtraTotalDamageRatio = 0.15;
    static extraTotalDamageRatioLevelMx = 0.015;
    static duration = GEM_CONSTANTS.DOT_DEFAULT_DURATION;
    static dotInterval = GEM_CONSTANTS.DOT_DEFAULT_INTERVAL;
    static get stasisDescription() {
        return `对敌人造成非神圣伤害时会在 ${this.duration / 1000} 秒内造成相当于这次伤害 ${MathUtils.roundWithFixed(this.baseExtraTotalDamageRatio * 100, 1)}%（+${MathUtils.roundWithFixed(this.extraTotalDamageRatioLevelMx * 100, 1)}%/等级）的神圣伤害，这个效果可以叠加`;
    }
    static get __base_description() {
...
class GemOfAnger extends GemBase {
    ...
    static gemName = '愤怒宝石';
    static price = 30000;
    static maxLevel = 2000;
    static imgSrc = GEM_IMAGES.GemOfAnger;
    static baseDamageAdditionPerEnemy = 0.02;
    static damageAdditionPerEnemyLevelMx = 0.012;
    static get stasisDescription() {
        return `攻击范围内的每个敌人都将使你的攻击力提高 ${MathUtils.roundWithFixed(this.baseDamageAdditionPerEnemy * 100, 2)}%（+${MathUtils.roundWithFixed(this.damageAdditionPerEnemyLevelMx * 100, 2)}%/等级）`;
    }
    static get __base_description() {
...
class BrokenPieces extends GemBase {
    ...
    static price = 620000;
    static maxLevel = 1000;
    static imgSrc = GEM_IMAGES.BrokenPieces;
    static baseArmorDecreaseStrength = 0.001;
    static armorDecreaseStrengthLevelMx = 0.0001;
    static baseExtraArmorBasedDamageRatio = 80;
    static extraArmorBasedDamageRatioLevelMx = 8;
    static get stasisDescription() {
        return (`每次击中都能摧毁敌人 ${MathUtils.roundWithFixed(this.baseArmorDecreaseStrength * 100, 1)}%（+${MathUtils.roundWithFixed(this.armorDecreaseStrengthLevelMx * 100, 2)}%/等级）的护甲，` +
            `并造成相当于敌人护甲总量的 ${MathUtils.roundWithFixed(this.baseExtraArmorBasedDamageRatio * 100, 0)}%（+${MathUtils.roundWithFixed(this.extraArmorBasedDamageRatioLevelMx * 100, 1)}%/等级）的额外伤害`);
    }
    static get __base_description() {
...
(function (DebuffType) {
    ...
    DebuffType["LIGHT_ECHO"] = "light_echo";
    DebuffType["SHOCK"] = "shock";
    DebuffType["TRANSFORM"] = "transform";
    DebuffType["IMPRISON"] = "imprison";
    DebuffType["FREEZE"] = "freeze";
    DebuffType["CONFUSE"] = "confuse";
    DebuffType["IMPRECATE"] = "imprecate";
    DebuffType["SLOW"] = "slow";
})(DebuffType || (DebuffType = {}));
var DebuffCategory;
(function (DebuffCategory) {
    DebuffCategory["DOT"] = "dot";
    DebuffCategory["CONTROL"] = "control";
    DebuffCategory["MODIFIER"] = "modifier";
})(DebuffCategory || (DebuffCategory = {}));
const DEBUFF_ICON_MAP = {
...
class RectangleBase extends Base {
    ...
    fillStyle;
    borderRadius;
    constructor(positionTL, positionBR, borderWidth, borderStyle, fillStyle, borderRadius) {
        super();
    ...
    renderBorder(context) {
        if (this.borderWidth > 0) {
            ...
            RenderUtils.renderRoundRect(context, this.cornerTL.x, this.cornerTL.y, this.width, this.height, this.borderRadius, false, true);
            context.restore();
        }
    }
    renderInside(context) {
        ...
        RenderUtils.renderRoundRect(context, this.cornerTL.x, this.cornerTL.y, this.width, this.height, this.borderRadius, true, false);
        context.restore();
    }
}
class CircleBase extends Base {
    ...
    borderWidth;
    borderStyle;
    constructor(position, radius, borderWidth, borderStyle) {
        super();
...
class AnimationSprite extends Base {
    ...
    lastRAF;
    isDead;
    constructor(img, columnCount, rowCount, frameRepetition) {
        super();
    ...
    renderOneFrame(ctx, positionTL, width, height, delay, endless, trusteeShippedClearing, recirculation, callback) {
        if (this.isFinish) {
            if (endless) {
                this.nextFrameIndex = 0;
            }
...
class HostedAnimationSprite extends Base {
    ...
    waitFrame;
    render;
    constructor(sp, pos, width, height, delay = 0, endless, waitFrame = 0) {
        super();
        this.sp = sp;
...
class TowerBase extends ItemBase {
    ...
    _killExtraPoint = 0;
    _onBossAtkRatio = 1;
    _onTrappedAtkRatio = 1;
    _angerGemAtkRatio = 1;
    _maxRangeAtkRatio = 1;
    _minRangeAtkRatio = 1;
    _eachMonsterDamageRatio = new Map();
    description = null;
    name = null;
    isSold = false;
    boundRecordDamage;
    ...
    levelUp(currentMoney) {
        ...
        if (this.price[this.level + 1] > currentMoney) {
            ...
            this.gameContext.playAnimation('level_up', new Position(this.position.x - this.radius, this.position.y - this.radius * 2), w, (w / dim.WIDTH) * dim.HEIGHT, GAME_CONFIG.LEVEL_UP_ANIMATION_SPEED);
    ...
    renderStatusBoard(bx1, _bx2, by1, _by2, showGemPanel, showMoreDetail, specifiedWidth) {
...
class MonsterBase extends ItemBase {
    ...
    shockLeakChance = 0;
    shockSource = null;
    transformDurationTick = 0;
    imprisonDurationTick = 0;
    freezeDurationTick = 0;
    imprecatedRatio = [];
    lastAbsDmg = 0;
    isBoss = false;
    isDead = false;
    ...
    renderStatusBoard(bx1, _bx2, by1, _by2, showGemPanel, showMoreDetail) {
        const data = {
            position: this.position,
            radius: this.radius,
            informationSeq: this.informationSeq,
...
class BulletBase extends ItemBase {
    ...
    hit(monster, magnification = 1, _monsters) {
...
class ButtonOnDom {
    ele;
    constructor(domOptions, appendOnToDom = true) {
...
class NormalArrow extends BulletBase {
    static BASE_VELOCITY = BULLET_VELOCITY.NORMAL_ARROW;
    critChance;
    critRatio;
    willTrap;
    trapDuration;
    ...
    constructor(position, atk, target, image, critChance, critRatio, trapChance, trapDuration, extraBV, isSecKill) {
        super(position, 8, 0, null, image, atk, NormalArrow.BASE_VELOCITY + (extraBV || 0), target);
...
class PenetratingArrow extends BulletBase {
    static BASE_VELOCITY = BULLET_VELOCITY.PENETRATING_ARROW;
    destination;
    hitMonsterIds = new Set();
    armorPenetration;
    damageMultiplier = 1;
    damageDecay;
    constructor(position, atk, target, image, armorPenetration = PENETRATING_ARROW_DEFAULTS.ARMOR_PENETRATION, damageDecay = PENETRATING_ARROW_DEFAULTS.DAMAGE_DECAY) {
    ...
    run(monsters) {
        ...
        for (const monster of monsters) {
            if (monster.isDead)
                continue;
            if (this.hitMonsterIds.has(monster.id))
                continue;
            if (!this.inRange(monster))
                continue;
            this.hit(monster);
...
class CannonBullet extends BulletBase {
    static BASE_VELOCITY = BULLET_VELOCITY.CANNON;
    aimPosition = null;
    explosionDmg;
    explosionRadius;
    burnDotDamage;
    ...
    burnDotDuration;
    ratioCalc;
    constructor(position, atk, target, _image, explosionDmg, explosionRadius, burnDotDamage, burnDotInterval, burnDotDuration, extraBV, ratioCalc) {
        super(position, BULLET_VISUAL_CONFIG.CANNON.radius, BULLET_VISUAL_CONFIG.CANNON.borderWidth, BULLET_VISUAL_CONFIG.CANNON.borderStyle, BULLET_VISUAL_CONFIG.CANNON.fill, atk, CannonBullet.BASE_VELOCITY + (extraBV || 0), target);
...
class ClusterBomb extends CannonBullet {
    constructor(position, atk, target, _image, explosionDmg, explosionRadius, burnDotDamage, burnDotInterval, burnDotDuration, extraBV, ratioCalc) {
        super(position, atk, target, _image, explosionDmg, explosionRadius, burnDotDamage, burnDotInterval, burnDotDuration, extraBV, ratioCalc);
...
class PoisonCan extends BulletBase {
    static BASE_VELOCITY = BULLET_VELOCITY.POISON;
    poisonAtk;
    poisonInterval;
    poisonDuration;
    constructor(position, atk, target, _image, poisonAtk, poisonInterval, poisonDuration, extraBV) {
        super(position, BULLET_VISUAL_CONFIG.POISON.radius, BULLET_VISUAL_CONFIG.POISON.borderWidth, BULLET_VISUAL_CONFIG.POISON.borderStyle, BULLET_VISUAL_CONFIG.POISON.fill, atk, PoisonCan.BASE_VELOCITY + (extraBV || 0), target);
...
class Blade extends BulletBase {
    static BASE_VELOCITY = BULLET_VELOCITY.BLADE;
    bounceTime;
    damageFadePerBounce;
    constructor(position, atk, target, image, bounceTime, damageFadePerBounce) {
        super(position, 4, 0, null, image, atk, Blade.BASE_VELOCITY, target);
...
class MysticBomb extends BulletBase {
    static BASE_VELOCITY = BULLET_VELOCITY.MYSTIC;
    triggerRadius;
    explosionDamage;
    lifetime;
    age = 0;
    triggered = false;
    constructor(position, atk, _target, _image, triggerRadius, explosionDamage, lifetime = MYSTIC_BOMB_DEFAULTS.LIFETIME) {
        super(position, BULLET_VISUAL_CONFIG.MYSTIC.radius, BULLET_VISUAL_CONFIG.MYSTIC.borderWidth, BULLET_VISUAL_CONFIG.MYSTIC.borderStyle, BULLET_VISUAL_CONFIG.MYSTIC.fill, atk, MysticBomb.BASE_VELOCITY, null);
    ...
    run(monsters) {
        ...
        for (const monster of monsters) {
            if (monster.isDead)
                continue;
            if (Position.distancePow2(monster.position, this.position) >= triggerRadiusSquared)
                continue;
            if (!this.triggered) {
                this.triggered = true;
    ...
    explode(monsters) {
        const positionTL = new Position(this.position.x - this.triggerRadius, this.position.y - this.triggerRadius);
        Game.callAnimation('explo_3', positionTL, this.triggerRadius * 2, this.triggerRadius * 2, 0.5, 0);
        const triggerRadiusSquared = this.triggerRadius * this.triggerRadius;
        for (const m of monsters) {
            if (m.isDead)
                continue;
            if (Position.distancePow2(m.position, this.position) >= triggerRadiusSquared)
                continue;
...
class CanvasManager {
    static CONTEXT_2D = '2d';
    static CONTEXT_BITMAP = 'bitmaprenderer';
    static DEFAULT_FONT = 'lighter 7px Game';
    static DEFAULT_CONTEXT_ID = 'bg';
    canvasElements = [];
    canvasContextMapping = new Map();
    offscreenCanvasMapping = new Map();
    _dpiCache = null;
    _offscreenSupport = null;
    get dpi() {
        return this._dpiCache ?? (this._dpiCache = window.devicePixelRatio);
    }
    get supportsOffscreen() {
    ...
    createCanvasInstance(id, style = {}, height, width, offDocument, wiredEvent = null, paintingOffScreenRenderingContextId = null) {
        ...
        const canvas = this._createOnscreen(id, dimensions.width, dimensions.height, style || {}, wiredEvent);
        let ctx;
        if (paintingOffScreenRenderingContextId) {
            ctx = this._setupBitmapRenderer(canvas, paintingOffScreenRenderingContextId);
        }
        else {
            const rawCtx = canvas.getContext(CanvasManager.CONTEXT_2D);
    ...
    _createOffscreen(id, width, height) {
        let canvas;
        ...
        const ctx = canvas.getContext(CanvasManager.CONTEXT_2D);
    ...
    _createOnscreen(id, width, height, style, eventHandler) {
    ...
    _setupBitmapRenderer(canvas, offscreenId) {
        ...
        if (this.supportsOffscreen) {
            const ctx = canvas.getContext(CanvasManager.CONTEXT_BITMAP);
            const osc = offscreen;
    ...
    _getDefaultContext() {
        if (!this.canvasContextMapping.has(CanvasManager.DEFAULT_CONTEXT_ID)) {
            throw new Error(`Default context "${CanvasManager.DEFAULT_CONTEXT_ID}" not found. Create the background canvas first.`);
        }
        return this.getContext(CanvasManager.DEFAULT_CONTEXT_ID);
    }
}
class EventManager {
    bindEvent(eventAndCallback, ele) {
        eventAndCallback.forEach(eac => (ele[eac.evtName] = eac.cb));
    }
}
function calcNormalMonsterRadius() {
    return Game.callGridSideSize() / MONSTER_CONFIG.NORMAL_RADIUS_DIVISOR - MONSTER_CONFIG.NORMAL_RADIUS_OFFSET;
}
function calcBossMonsterRadius() {
...
class Dummy extends MonsterBase {
    ...
    static sprSpd = 20;
    static rwd = (_lvl) => 0;
    static spd = (_lvl) => 0.001;
    static hth = (lvl) => (lvl + 1) * 4e8;
    static amr = (_lvl) => 0;
    constructor(position, image, level) {
        super(position, calcNormalMonsterRadius(), 0, null, image, level, Dummy.rwd, Dummy.spd, Dummy.hth, Dummy.amr);
...
class Swordman extends MonsterBase {
    ...
    static spd = (lvl) => Math.min(0.3 + lvl / 60, 1.15);
    static hth = (lvl) => 120 + lvl * 40;
    static amr = (lvl) => 3 + lvl / 8;
    constructor(position, image, level) {
        super(position, calcNormalMonsterRadius(), 0, null, image, level, Swordman.rwd, Swordman.spd, Swordman.hth, Swordman.amr);
        this.name = '邪恶的剑士';
...
class Axeman extends MonsterBase {
    ...
    static spd = (lvl) => Math.min(0.25 + lvl / 80, 1);
    static hth = (lvl) => 300 + lvl * 100;
    static amr = (lvl) => 5 + lvl / 6;
    constructor(position, image, level) {
        super(position, calcNormalMonsterRadius(), 0, null, image, level, Axeman.rwd, Axeman.spd, Axeman.hth, Axeman.amr);
        this.name = '蛮族斧手';
...
class LionMan extends MonsterBase {
    ...
    static spd = (lvl) => Math.min(0.38 + lvl / 70, 1.2);
    static hth = (lvl) => 580 + lvl * 122;
    static amr = (lvl) => 22 + lvl / 5;
    constructor(position, image, level) {
        super(position, calcNormalMonsterRadius(), 0, null, image, level, LionMan.rwd, LionMan.spd, LionMan.hth, LionMan.amr);
        this.name = '狮人';
...
class DemonSpawn extends MonsterBase {
    ...
    static sprSpd = 4;
    static rwd = (lvl) => 5 * lvl + 10;
    static spd = (lvl) => Math.min(0.4 + lvl / 50, 1.3);
    static hth = (lvl) => 50 + lvl * 20;
    static amr = (lvl) => lvl / 10;
    constructor(position, image, level) {
        super(position, Game.callGridSideSize() / MONSTER_CONFIG.DEMON_SPAWN_RADIUS_DIVISOR, 0, null, image, level, DemonSpawn.rwd, DemonSpawn.spd, DemonSpawn.hth, DemonSpawn.amr);
        this.name = '恶魔崽';
...
class HighPriest extends MonsterBase {
    ...
    static spd = (_lvl) => 0.11;
    static hth = (lvl) => 14400 + lvl * 8000;
    static amr = (_lvl) => 14;
    static healingInterval = (_lvl) => 5000;
    static healingPower = (lvl) => 40 * (Math.floor(lvl / 2) + 1);
    static healingRange = (_lvl) => 30;
    lastHealTime = performance.now();
    _currentHealInterval;
    constructor(position, image, level) {
        super(position, calcBossMonsterRadius(), 0, null, image, level, HighPriest.rwd, HighPriest.spd, HighPriest.hth, HighPriest.amr);
        this.isBoss = true;
        this.name = '龙人萨满';
        this.type = '首领';
...
class Devil extends MonsterBase {
    ...
    static spd = (_lvl) => 0.14;
    static hth = (lvl) => 15500 + lvl * 13000;
    static amr = (lvl) => 32 + lvl;
    static summonInterval = () => 7000;
    lastSummonTime = performance.now();
    constructor(position, image, level) {
        super(position, Game.callGridSideSize() / MONSTER_CONFIG.DEVIL_RADIUS_DIVISOR + MONSTER_CONFIG.DEVIL_RADIUS_OFFSET, 0, null, image, level, Devil.rwd, Devil.spd, Devil.hth, Devil.amr);
        this.isBoss = true;
        this.name = '地狱之王';
        this.type = '首领';
...
function createPriceProxy(basePrice, maxLevel) {
    return new Proxy({}, {
        get(_t, p) {
            if (p === 'length')
                return maxLevel;
...
const towerCtors = [
    ...
    },
    {
        ...
        h: (_lvl) => Infinity,
        s: (_lvl) => 0,
        sr: (lvl) => MathFx.naturalLogFx(0.1, 0.12)(lvl)
    },
    {
        dn: '毒气塔',
        c: 'PoisonTower',
...
class _TowerManager {
    ...
    static TestTower;
    static CannonShooter;
    static MaskManTower;
    static FrostTower;
    static PoisonTower;
    static TeslaTower;
    static BlackMagicTower;
    static LaserTower;
    static CarrierTower;
    static EjectBlade;
    towers = [];
...
const BLACK_MAGIC_TOWER_UPGRADES = [
    {
        level: 5,
        rankUp: true,
        name: '奥术魔法塔',
...
function applyLevelUpEffects(tower, upgrades, towerConfig) {
    ...
    if (upgrade.imageKey && towerConfig[upgrade.imageKey]) {
        tower.image = Game.callImageBitMap(towerConfig[upgrade.imageKey]);
    }
...
class _ColossusLaser {
    static animationW = COLOSSUS_LASER_CONSTANTS.ANIMATION_WIDTH;
    static animationH = COLOSSUS_LASER_CONSTANTS.ANIMATION_HEIGHT;
    static animationName = COLOSSUS_LASER_CONSTANTS.ANIMATION_NAME;
    static animationSpeed = COLOSSUS_LASER_CONSTANTS.ANIMATION_SPEED;
    sx;
    sy;
    ep;
    swipeVector;
    ...
    easingFx;
    fulfilled = false;
    constructor(startPos, endPos, lineWidth, duration, swipeVector, easingFunc, lineStyleOuter, lineStyleInner) {
        this.sx = startPos.x;
        this.sy = startPos.y;
        this.ep = endPos.copy();
...
class _Jet extends TowerBase {
    ...
    actMode = _Jet.JetActMode.autonomous;
    weaponMode = 1;
    destinationPosition;
    inner_desc_init = '航母的载机\n+ 机动性极强\n+ 拥有 15mm 速射机枪和 30mm 反装甲机炮两种武器';
    carrierTower;
    constructor(position, image, _bulletImage, radius, carrierTower) {
...
class TestTower extends TowerBase {
    ...
    constructor(position, image, _bulletImage, radius) {
...
class CannonShooter extends TowerBase {
    ...
    static rankUpDesc3 = '\n+ 命中后向四周抛出小型炸弹';
    static rankUpDesc4 = '\n+ 小型炸弹将分裂两次';
    levelEpdRngFx = TowerManager.CannonShooter.expr;
    levelEpdAtkFx = TowerManager.CannonShooter.expatk;
    levelBrnAtkFx = TowerManager.CannonShooter.bdatk;
    levelBrnItvFx = TowerManager.CannonShooter.bditv;
    levelBrnDurFx = TowerManager.CannonShooter.bddur;
    _extraExplosionDamage = 0;
    extraExplosionRange = 0;
    extraExplosionDamageRatio = 1;
    extraExplosionRangeRatio = 1;
    extraRange = 0;
    extraBulletV = 0;
    _dynamicExplosionDamage = false;
    inner_desc_init = '发射火炮，在命中后会爆炸\n+ 附加灼烧效果';
    constructor(position, image, _bulletImage, radius) {
        super(position, radius, 1, TOWER_COLORS.CANNON_RANK_0, image, TowerManager.CannonShooter.p, TowerManager.CannonShooter.a, TowerManager.CannonShooter.h, TowerManager.CannonShooter.s, TowerManager.CannonShooter.r);
        this.bulletCtorName = TowerManager.CannonShooter.bctor;
        this.name = TowerManager.CannonShooter.dn;
        this.description = this.inner_desc_init;
    }
    get extraExplosionDamage() {
    ...
    set extraExplosionDamage(value) {
...
class MaskManTower extends TowerBase {
    ...
    trapDuration = 0;
    extraBulletV = 0;
    _dynamicExtraHaste = false;
    inner_desc_init = '每次向多个敌人射出箭矢\n+ 有几率暴击\n+ 拥有固定 30%的护甲穿透';
    critChance = ARCHER_CONSTANTS.CRIT_CHANCE_BASE;
    critDamageRatio = ARCHER_CONSTANTS.CRIT_DAMAGE_BASE;
    secKillChance = 0;
    constructor(position, image, bulletImage, radius) {
        super(position, radius, 1, TOWER_COLORS.ARCHER_RANK_0, image, TowerManager.MaskManTower.p, TowerManager.MaskManTower.a, TowerManager.MaskManTower.h, TowerManager.MaskManTower.s, TowerManager.MaskManTower.r);
...
class FrostTower extends TowerBase {
    ...
    static rankUpDesc3 = '\n+ 冻结能力加强';
    canInsertGem = false;
    levelSprFx = TowerManager.FrostTower.sr;
    inner_desc_init = '在自身周围形成一个圆形的减速场\n- 无法攻击\n- 无法镶嵌传奇宝石';
    armorDecreasingStrength = FROST_CONSTANTS.ARMOR_DECREASING_STRENGTH;
    lastFreezeTime = performance.now();
    freezeInterval = Infinity;
    freezeDuration = 0;
    freezeEffectLevel = 0;
    constructor(position, image, _bulletImage, radius) {
        super(position, radius, 1, TOWER_COLORS.FROST, image, TowerManager.FrostTower.p, TowerManager.FrostTower.a, TowerManager.FrostTower.h, TowerManager.FrostTower.s, TowerManager.FrostTower.r);
        this.canInsertGem = false;
    ...
    setFreezeEffectLevel(level) {
        this.freezeEffectLevel = level;
    }
    applyFreezeEffect(monsters) {
        if (!this.canFreeze)
            return;
...
class PoisonTower extends TowerBase {
    levelPatkFx = TowerManager.PoisonTower.patk;
    levelPitvFx = TowerManager.PoisonTower.pitv;
    levelPdurFx = TowerManager.PoisonTower.pdur;
    extraBulletV = 0;
    inner_desc_init = '发射毒气弹持续杀伤，总是积极切换目标\n+ 攻击速度很快\n+ 附加中毒效果\n+ 无视防御的伤害';
    constructor(position, image, _bulletImage, radius) {
        super(position, radius, 1, TOWER_COLORS.POISON, image, TowerManager.PoisonTower.p, TowerManager.PoisonTower.a, TowerManager.PoisonTower.h, TowerManager.PoisonTower.s, TowerManager.PoisonTower.r);
...
class TeslaTower extends TowerBase {
    ...
    extraRange = 0;
    extraHaste = 0;
    shockDuration = TESLA_CONSTANTS.SHOCK_DURATION_BASE;
    shockChargingChance = TESLA_CONSTANTS.SHOCK_CHARGING_CHANCE_BASE;
    shockChargingPowerRatio = TESLA_CONSTANTS.SHOCK_CHARGING_POWER_RATIO_BASE;
    shockLeakingChance = TESLA_CONSTANTS.SHOCK_LEAKING_CHANCE_BASE;
    monsterShockingRenderingQueue = [];
    inner_desc_init = '向周围小范围释放电击造成中等伤害\n+ 有几率使目标带电';
    constructor(position, image, _bulletImage, radius) {
        super(position, radius, 1, TOWER_COLORS.TESLA_RANK_0, image, TowerManager.TeslaTower.p, TowerManager.TeslaTower.a, TowerManager.TeslaTower.h, TowerManager.TeslaTower.s, TowerManager.TeslaTower.r);
...
class BlackMagicTower extends TowerBase {
    ...
    levelIdeFx = TowerManager.BlackMagicTower.ide;
    levelIdrFx = TowerManager.BlackMagicTower.idr;
    imprecationPower = 0;
    imprecationHaste = 1;
    extraPower = 0;
    POTCHD = 0;
    inner_desc_init = '释放强力魔法\n- 准备时间非常长\n+ 附加诅咒，使目标受到的伤害提高\n+ 每次击杀增加 10 攻击力并提高 5% 攻击速度（最多提高 1600%）';
    constructor(position, image, _bulletImage, radius) {
        super(position, radius, 1, TOWER_COLORS.MAGIC_RANK_0, image, TowerManager.BlackMagicTower.p, TowerManager.BlackMagicTower.a, TowerManager.BlackMagicTower.h, TowerManager.BlackMagicTower.s, TowerManager.BlackMagicTower.r);
...
class LaserTower extends TowerBase {
    ...
    static rankUpDesc4 = '\n+ 所有属性得到增强';
    lasers = [];
    levelFlameAtkFx = TowerManager.LaserTower.fatk;
    levelFlameWidthFx = TowerManager.LaserTower.fw;
    levelLaserSwipeDistanceFx = TowerManager.LaserTower.lsd;
    extraFlameDamage = 0;
    extraLuminousDamage = 0;
    extraLaserTransmitter = 0;
    extraFlameWidth = 0;
    extraRange = 0;
    lineStyles = LASER_CONSTANTS.LINE_STYLES;
    inner_desc_init = '发射激光，横扫大面积的目标，造成范围的火焰伤害';
    constructor(position, image, _bulletImage, radius) {
        super(position, radius, 1, TOWER_COLORS.LASER_RANK_0, image, TowerManager.LaserTower.p, TowerManager.LaserTower.a, TowerManager.LaserTower.h, TowerManager.LaserTower.s, TowerManager.LaserTower.r);
    ...
    produceBullet(_i, monsters) {
        ...
        const swipeVector = _.maxBy(_.range(0, 360, 30).map(d => new PolarVector(this.LSD, d)), sv => monsters.filter(mst => targetPos
            .copy()
            .move(sv.copy().normalize().multiply(moveRatio * (arcTime - 1) * hitRadius))
            .equal(mst.position, 1.2 * hitRadius)).length).dithering((1 / 30) * Math.PI);
        this.lasers.push(new LaserTower.Laser(this.position, this.target.position, this.LRW, 500, swipeVector, EaseFx.linear, this.laserLineStyle[0], this.laserLineStyle[1]));
...
class CarrierTower extends TowerBase {
    ...
    jetCountMap = new Map();
    jetCount = 0;
    levelSpdFx = TowerManager.CarrierTower.spd;
    levelKcFx = TowerManager.CarrierTower.child;
    inner_desc_init = '自身无法攻击，释放搭载的载机进行战斗\n使用 [F1] 切换载机的自主/受控模式\n使用 [Q] 切换载机的武器\n+ 载机继承自身属性\n+ 可以对任意位置进行机动打击';
    constructor(position, image, _bulletImage, radius) {
        super(position, radius, 1, TOWER_COLORS.CARRIER, image, TowerManager.CarrierTower.p, TowerManager.CarrierTower.a, TowerManager.CarrierTower.h, TowerManager.CarrierTower.s, TowerManager.CarrierTower.r);
...
class EjectBlade extends TowerBase {
    inner_desc_init = '发射可以在敌人之间弹射的飞刃\n+ 每次弹射将使伤害衰减，升级可以减少衰减的程度';
    levelBtFxFx = TowerManager.EjectBlade.bt;
    levelDfpbFx = TowerManager.EjectBlade.dfpb;
    constructor(position, image, bulletImage, radius) {
        super(position, radius, 1, TOWER_COLORS.BLADE, image, TowerManager.EjectBlade.p, TowerManager.EjectBlade.a, TowerManager.EjectBlade.h, TowerManager.EjectBlade.s, TowerManager.EjectBlade.r);
...
class ImageManager {
    bitmapMapping = new Map();
    spriteMapping = new Map();
    onPlaySprites = [];
    play(ctx) {
...
(function (WaveState) {
    WaveState["IDLE"] = "IDLE";
    WaveState["WAITING_FOR_START"] = "WAITING";
    WaveState["SPAWNING"] = "SPAWNING";
    WaveState["RESTING"] = "RESTING";
...
class Wave {
    ...
    _waveNumber;
    _restTicks;
    _currentSummonIndex = 0;
    _spawnedInCurrentConfig = 0;
    _lastSpawnTick = -Infinity;
    _totalSpawned = 0;
    _totalMonsters;
    constructor(waveNumber, summons, restTicks) {
...
class WaveManager {
    ...
    _restStartTick = 0;
    _autoStart = WAVE_CONFIG.DEFAULT_AUTO_START;
    _spawnCallback = null;
    ...
    getRestRemainingTicks(currentTick) {
        if (this._state !== WaveState.RESTING)
            return 0;
        const wave = this._waves[this._currentWaveIndex];
        if (!wave)
...
class WaveFactory {
    static NORMAL_MONSTERS = ['Swordman', 'Axeman', 'LionMan'];
    static BOSS_MONSTERS = ['Devil', 'HighPriest'];
    static validateWaveDefinition(def) {
    ...
    static createStandardWaveDefinitions() {
        const definitions = [];
        for (let i = 1; i <= 10; i++) {
    ...
    static createWaveDefinition(waveNumber) {
    ...
    static createEndlessWaveDefinition(waveNumber) {
...
class WaveUIManager {
    ...
    _initialized = false;
    init() {
        this._panelEl = document.getElementById('wave_panel');
        this._waveNumberEl = document.getElementById('wave_number');
        this._waveTotalEl = document.getElementById('wave_total');
        this._progressBarEl = document.getElementById('wave_progress_bar');
        this._restTimerEl = document.getElementById('wave_rest_timer');
        this._startBtnEl = document.getElementById('wave_start_btn');
        this._autoStartEl = document.getElementById('wave_auto_start');
    ...
    _updateRestingUI(manager, currentTick) {
        const wave = manager.getCurrentWave();
        if (!wave)
            return;
...
class GamePathfinder {
    ...
    findPath(startPos, grids, wallBoundary) {
        const gridInfo = this.getGridInfoAtPosition(startPos, Infinity);
        if (!gridInfo)
            return [];
        const cacheKey = `${gridInfo.gridX}|${gridInfo.gridY}`;
    ...
    wouldBlockAllPaths(gridX, gridY, grids, wallBoundary) {
    ...
    _getOrCreateGraph(grids, wallBoundary) {
        if (!this._graphCache || this._graphDirty) {
            const gridsWithWall = wallBoundary.concat(grids.map(row => [0, ...row, 0]).concat(wallBoundary));
...
class GameRenderer {
    ...
    _mouseCtx;
    _offscreenCtx;
    _useClassicRenderStyle;
    constructor(canvasManager, imageManager) {
        this._canvasManager = canvasManager;
        this._imageManager = imageManager;
        this._useClassicRenderStyle = !('OffscreenCanvas' in window);
    }
    initCanvasLayers(_leftAreaWidth, _leftAreaHeight) {
        this._canvasManager.createCanvasInstance('off_screen_render', undefined, undefined, undefined, true);
        this._offscreenCtx = this._canvasManager.getContext('off_screen_render');
        this._canvasManager.createCanvasInstance('main', { zIndex: CANVAS_Z_INDEX.MAIN }, undefined, undefined, false, null, 'off_screen_render');
        this._mainCtx = this._canvasManager.getContext('main');
        this._canvasManager.createCanvasInstance('tower', { zIndex: CANVAS_Z_INDEX.TOWER });
        this._towerCtx = this._canvasManager.getContext('tower');
        this._canvasManager.createCanvasInstance('mouse', { zIndex: CANVAS_Z_INDEX.MOUSE });
        this._mouseCtx = this._canvasManager.getContext('mouse');
        this._canvasManager.createCanvasInstance('bg', { zIndex: CANVAS_Z_INDEX.BACKGROUND });
        this._backgroundCtx = this._canvasManager.getContext('bg');
    }
    get backgroundCtx() {
    ...
    renderBackground(config) {
        ...
        if (config.isTestMode) {
            this._canvasManager.refreshText('[ Test Mode ]', null, new Position(10, 15), new Position(8, 15), 120, 26, 'rgba(230,204,55,1)', true, '10px SourceCodePro');
        }
        this._canvasManager.refreshText('鼠标点击选取建造，连点两次鼠标右键出售已建造的塔', null, new Position(config.leftAreaWidth + UI_LAYOUT.TOWER_SELECTOR_MARGIN, 30), new Position(config.leftAreaWidth + UI_LAYOUT.TOWER_SELECTOR_MARGIN - 2, 10), config.rightAreaWidth, 26, 'rgba(24,24,24,1)', true, '14px Game');
        this._canvasManager.refreshText('出现详情时按[Ctrl]切换详细信息和说明', null, new Position(config.leftAreaWidth + UI_LAYOUT.TOWER_SELECTOR_MARGIN, 70), new Position(config.leftAreaWidth + UI_LAYOUT.TOWER_SELECTOR_MARGIN - 2, 50), config.rightAreaWidth, 26, 'rgba(24,24,24,1)', true, '14px Game');
    }
    renderInfoLabels() {
    ...
    renderMoney(money) {
        ...
        this._canvasManager.refreshText(FormatUtils.formatterUs.format(money), null, new Position(ax, ay), new Position(ax - 4, ay - 20), 160, 26, 'rgb(24,24,24)', true, '14px Game');
    }
    renderLife(life) {
        const ax = innerWidth - 160;
        const ay = innerHeight - 40;
        this._canvasManager.refreshText(life + '', null, new Position(ax, ay), new Position(ax - 4, ay - 20), 160, 26, 'rgb(24,24,24)', true, '14px Game');
    }
    renderGemPoint(gemPoints) {
        const ax = innerWidth - 160;
        const ay = innerHeight - 70;
    ...
    renderGameStats(totalDamage, totalKill, bornStamp) {
        ...
        const ay2 = ay1 - 30;
        const ay3 = ay2 - 30;
        this._canvasManager.refreshText(`DPS    ${DPS}`, null, new Position(ax, ay1), new Position(ax - 4, ay1 - 20), 190, 26, 'rgb(24,24,24)', true, '14px Game');
        this._canvasManager.refreshText(`总伤害    ${DMG}`, null, new Position(ax, ay2), new Position(ax - 4, ay2 - 20), 190, 26, 'rgb(24,24,24)', true, '14px Game');
        this._canvasManager.refreshText(`总击杀    ${TK}`, null, new Position(ax, ay3), new Position(ax - 4, ay3 - 20), 190, 26, 'rgb(24,24,24)', true, '14px Game');
    }
    renderDebugInfo(metrics) {
    ...
    renderStandardText(text, bx, by, maxWidth, color = 'rgb(2,2,2)', fSize = 10) {
        this._canvasManager.refreshText(text, null, new Position(bx + 4, by + fSize + 5), new Position(bx, by), maxWidth, 12 + fSize, color, true, `${fSize}px SourceCodePro`);
    }
    createPerformanceWrapper(fn, metricsName, xAxis, yAxis, bufferLength = 120, height = 50, maxWidth = 120, maxV = 50, warningV = 12, dangerV = 16.667) {
        const timeBuffer = new Float64Array(bufferLength).fill(-1);
        return () => {
            const runStart = performance.now();
            fn();
            const metrics = performance.now() - runStart;
            this.renderStandardText(`[ ${metricsName} ${MathUtils.roundWithFixed(metrics, 3)} ms ]`, xAxis, yAxis, maxWidth);
            const actualLength = MathUtils.typedArrayPush(timeBuffer, metrics);
            if (actualLength === timeBuffer.length) {
                this.renderStandardText(`[ ${metricsName} avg ${MathUtils.roundWithFixed(timeBuffer.reduce((c, p) => c + p, 0) / actualLength, 3)} ms ]`, xAxis, yAxis + 20, maxWidth);
            }
            else {
                this.renderStandardText(`[ ${metricsName} avg - ms ]`, xAxis, yAxis + 20, maxWidth);
            }
            RenderUtils.renderStatistic(metricsName, this._backgroundCtx, timeBuffer, new Position(xAxis, yAxis + height), timeBuffer.length, height + 2, maxV, warningV, dangerV);
        };
    }
    clearMouseLayer() {
    ...
    get useClassicRenderStyle() {
        return this._useClassicRenderStyle;
    }
}
class GameEventHandler {
    _game;
    _lastRightClickTime = -1000;
    _lastMouseMovePosition = Position.ORIGIN;
    _detailFunctionKeyDown = false;
    _selectedTowerTypeToBuild = null;
    _statusBoardOnTower = null;
    constructor(game) {
        this._game = game;
    }
    get lastMouseMovePosition() {
        return this._lastMouseMovePosition;
    }
    get detailFunctionKeyDown() {
        return this._detailFunctionKeyDown;
    }
    get selectedTowerTypeToBuild() {
        return this._selectedTowerTypeToBuild;
    }
    set selectedTowerTypeToBuild(value) {
        this._selectedTowerTypeToBuild = value;
    }
    get statusBoardOnTower() {
        return this._statusBoardOnTower;
    }
    handleLeftClick(mousePos, midSplitLineX, towerForSelect, towers, independentTowers, money, gridSize, pathfinder, grids, wallBoundary, monsters, onBuildTower, onUpdateMoney, mouseCtx) {
        if (this._selectedTowerTypeToBuild) {
            this._handleBuildModeClick(mousePos, midSplitLineX, money, gridSize, pathfinder, grids, wallBoundary, monsters, towers, onBuildTower);
            return;
        }
    ...
    handleRightClick(mousePos, towers, onRemoveTower) {
        const now = performance.now();
    ...
    createMouseMoveHandler(midSplitLineX, towerForSelect, getTowers, getIndependentTowers, getMonsters, mouseCtx, pathfinder, gridSize) {
        return _.throttle((e) => {
            ...
            if (this._selectedTowerTypeToBuild) {
                if (mousePos.x < midSplitLineX) {
                    ...
                    if (gridInfo) {
                        ...
                        ctx.fillRect(gridInfo.centerX - gridSize / 2, gridInfo.centerY - gridSize / 2, gridSize, gridSize);
                    ...
                    TowerBase.prototype.renderRange.call({ position: mousePos, Rng: this._selectedTowerTypeToBuild.__rng_lv0 }, mouseCtx);
                }
                return;
            }
            if (e.offsetX > midSplitLineX) {
                ...
                if (selectedT) {
                    ...
                    }, 0, midSplitLineX, 0, innerHeight, false, false);
                }
                else {
    ...
    handleKeyDown(e, towerForSelect, isTestMode, startAndPauseButton, onSpawnDummy, onSpawnDummyBatch, midSplitLineX, onLeftClick) {
        ...
        switch (e.key) {
            case 'c':
                ...
                break;
            case ' ':
                if (startAndPauseButton)
                    startAndPauseButton.onMouseClick();
                break;
            case 'h':
            case 'H': {
    ...
    _handleBuildModeClick(mousePos, midSplitLineX, money, gridSize, pathfinder, _grids, wallBoundary, monsters, towers, onBuildTower) {
        if (!this._selectedTowerTypeToBuild)
            return;
        const gridInfo = pathfinder.getGridInfoAtPosition(mousePos, midSplitLineX);
        if (!gridInfo) {
        ...
        this._selectedTowerTypeToBuild.__re_render(0);
        onBuildTower(mousePos, this._selectedTowerTypeToBuild.__ctor_name, this._selectedTowerTypeToBuild.__inner_img_u, this._selectedTowerTypeToBuild.__inner_b_img_u, -this._selectedTowerTypeToBuild.__init_price[0]);
        this._selectedTowerTypeToBuild = null;
    }
    _handleRightMenuClick(mousePos, towerForSelect) {
        const selectedT = towerForSelect.find(tfs => tfs.position.equal(mousePos, tfs.radius));
        if (selectedT) {
    ...
    _handleGameAreaClick(mousePos, towers, _independentTowers, money, midSplitLineX, onUpdateMoney, mouseCtx) {
        const selectedT = towers.find(t => t.position.equal(mousePos, t.radius));
        if (selectedT) {
    ...
    _hasMonsterAtGrid(gridX, gridY, monsters, gridSize) {
    ...
    _hasTowerAtGrid(gridX, gridY, towers, gridSize) {
...
class GameUIManager {
    _startPauseButton = null;
    _speedControlButton = null;
    _testModeControls = [];
    get startPauseButton() {
    ...
    initTestModeUI(config) {
        ...
        const row1 = DomUtils.generateRow(document.body, null, {
            style: {
                position: 'fixed',
                top: buttonTopB,
                left: config.leftAreaWidth + UI_LAYOUT.TOWER_SELECTOR_MARGIN + 'px',
        ...
        const row2 = DomUtils.generateRow(document.body, null, {
            style: {
                position: 'fixed',
                top: buttonTopC,
                left: config.leftAreaWidth + UI_LAYOUT.TOWER_SELECTOR_MARGIN + 'px',
    ...
    static showPauseOverlay(onResume) {
    ...
    static hidePauseOverlay() {
        const overlay = document.getElementById('pause_overlay');
        if (overlay)
            overlay.style.display = 'none';
    }
    static showGameOverOverlay(stats, onRestart) {
    ...
    static showVictoryOverlay(stats, onEndless, onRestart) {
...
class GameContextAdapter {
    ...
    playAnimation(name, pos, w, h, speed, delay, fps) {
...
class Game extends Base {
    static updateGemPoint;
    static callDiagonalLength;
    static callAnimation;
    static callImageBitMap;
    static callCanvasContext;
    static callBoundaryPosition;
    static callGridSideSize;
    static callMidSplitLineX;
    ...
    static callMoney;
    static callRemoveTower;
    static callTowerFactory;
    static callTowerList;
    static callIndependentTowerList;
    static callMonsterList;
    static callMonsterSpawn;
    static callOriginPosition;
    static callDestinationPosition;
    static callChangeF1Mode;
    static callChangeCarrierWeaponMode;
    ...
    _bornStamp;
    _setBornStamp = _.once(() => {
        this._bornStamp = performance.now();
    });
    _isTestMode;
    _isDummyTestMode;
    _tickRate = GAME_TIMING.TICK_RATE;
    _count;
    _stepDivide;
    _updateTick = 0;
    _renderTick = 0;
    _towerNeedRender = true;
    _gridColumns;
    _gridRows;
    _originGrid;
    _destinationGrid;
    ...
    get _objectCount() {
        return (this._imageManager.onPlaySprites.length +
    ...
    set _updateSpeedRatio(v) {
        this._speedRatio = v;
        MainLoop.setSimulationTimestep(1000 / this._tickRate / v);
    }
    get _updateSpeedRatio() {
        return this._speedRatio;
    }
    _initStaticAccessors() {
    ...
    _initEventBindings() {
        this._eventManager.bindEvent([
            {
                evtName: 'onkeydown',
                cb: (e) => this._handleKeyDown(e)
            }
        ], document);
        const reactDivEle = this._uiManager.createReactLayer();
        const mouseMoveHandler = this._eventHandler.createMouseMoveHandler(this._midSplitLineX, this._towerForSelect, () => this._towerManager.towers, () => this._towerManager.independentTowers, () => this._monsterManager.monsters, this._renderer.mouseCtx, this._pathfinder, this._gridSize);
        this._eventManager.bindEvent([
            {
                evtName: 'onmousedown',
                cb: (e) => {
    ...
    _handleLeftClick(mousePos) {
        this._eventHandler.handleLeftClick(mousePos, this._midSplitLineX, this._towerForSelect, this._towerManager.towers, this._towerManager.independentTowers, this._money, this._gridSize, this._pathfinder, this._grids, this._wallBoundary, this._monsterManager.monsters, (pos, ctorName, imgName, bImgName, price) => this._buildTower(pos, ctorName, imgName, bImgName, price), delta => this.updateMoney(delta), this._renderer.mouseCtx);
    }
    _handleRightClick(mousePos) {
        this._eventHandler.handleRightClick(mousePos, this._towerManager.towers, tower => this._removeTower(tower));
    }
    _handleKeyDown(e) {
        this._eventHandler.handleKeyDown(e, this._towerForSelect, this._isTestMode, this._uiManager.startPauseButton, () => this._spawnDummy(), () => this._spawnDummyBatch(), this._midSplitLineX, () => this._handleLeftClick(this._eventHandler.lastMouseMovePosition));
    }
    _enableDummyTestMode() {
        if (!this._isDummyTestMode)
            this._isDummyTestMode = true;
    }
    _spawnDummy() {
        this._enableDummyTestMode();
        this._spawnMonster(100, this._originPosition
            .copy()
            .move(new PolarVector(this._gridSize, 0))
            .dithering(this._gridSize, this._gridSize / 2), 'Dummy');
    }
    _spawnDummyBatch() {
        this._enableDummyTestMode();
        for (let i = 0; i < 100; i++) {
    ...
    _buildTower(pos, ctorName, imgName, bImgName, price) {
    ...
    _initWaveSystem() {
        ...
        waveManager.setSpawnCallback((monsterName, level) => {
            this._spawnMonster(level, this._originPosition.copy(), monsterName);
        });
        waveManager.setRewardCallback((amount) => {
    ...
    updateMoney(delta, _happenedPosition) {
        this._money += delta;
        if (this._isTestMode)
            this._money = GAME_CONFIG.TEST_MODE_MONEY;
    }
    emitMoney(delta, _happenedPosition) {
    ...
    render() {
        ...
        if (this._towerNeedRender) {
            ...
            this._towerManager.render(this._renderer.towerCtx);
